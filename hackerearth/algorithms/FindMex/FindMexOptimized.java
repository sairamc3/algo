...
class FindMex {
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int i = sizeOfArray - 1; i > 0; i--) {
            ...
            if (mexForWhole > inputList.get(i)) {

                if (count.containsKey(inputList.get(i))) {

                    if (count.get(inputList.get(i)) > 1) {
                        Integer noOfInstances = count.get(inputList.get(i));
                        count.remove(inputList.get(i));
                        count.put(inputList.get(i), noOfInstances - 1);
                    } else {
